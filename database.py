...
def ensure_schema_upgrade() -> None:
    """
    Applies small non-destructive schema upgrades required by newer features.
    This avoids runtime 500s when the app code is ahead of the DB structure.
    """
    upgrades = [
        ...
        (
            "notices",
            "category",
            "ENUM('notice','internship','job') NOT NULL DEFAULT 'notice'",
        ),
        ...
        (
            "documents",
            "category",
            "ENUM('document','internship','job') NOT NULL DEFAULT 'document'",
        ),
